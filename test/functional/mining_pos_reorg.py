...
from test_framework.test_framework import sssestFramework
...
class ReorgStakeTest(sssestFramework):
    ...
    def check_money_supply(self, expected_SSS, expected_zsss):
        g_info = [self.nodes[i].getinfo() for i in range(self.num_nodes)]
        # verify that nodes have the expected SSS and zSSS supply
        for node in g_info:
            assert_equal(node['moneysupply'], DecimalAmt(expected_SSS))
            for denom in node['zSSSsupply']:
                assert_equal(node['zSSSsupply'][denom], DecimalAmt(expected_zsss[denom]))
    ...
    def run_test(self):
        ...
        # Check SSS and zSSS supply at the beginning
        # ------------------------------------------
        # zSSS supply: 2 coins for each denomination
        expected_zsss_supply = {
            "1": 2,
            "5": 10,
            "10": 20,
            "50": 100,
        ...
        # SSS supply: block rewards minus burned fees for minting
        expected_money_supply = 250.0 * 330 - 16 * 0.01
        self.check_money_supply(expected_money_supply, expected_zsss_supply)
        ...
        assert_raises_rpc_error(-26, "bad-txns-invalid-zsss",
        ...
        # Verify that SSS and zSSS supplies were properly updated after the spends and reorgs
        self.log.info("Check SSS and zSSS supply...")
        ...
        expected_zsss_supply[str(spent_coin_0)] -= spent_coin_0
        expected_zsss_supply[str(spent_coin_1)] -= spent_coin_1
        expected_zsss_supply["total"] -= (spent_coin_0 + spent_coin_1)
        self.check_money_supply(expected_money_supply, expected_zsss_supply)
