#!/usr/bin/env python3
# Copyright (c) 2019 The sss developers
...
class ZerocoinSpendTest(sssTestFramework):
    ...
    def run_test(self):
        ...
        def check_balances(denom, zsss_bal, SSS_bal):
            zsss_bal -= denom
            assert_equal(self.nodes[2].getzerocoinbalance()['Total'], zsss_bal)
            SSS_bal += denom
            wi = self.nodes[2].getwalletinfo()
            assert_equal(wi['balance'] + wi['immature_balance'], SSS_bal)
            return zsss_bal, SSS_bal
        ...
        zsss_balance = self.nodes[2].getzerocoinbalance()['Total']
        assert_equal(balance, DecimalAmt(13833.92))
        assert_equal(zsss_balance, 6666)
        ...
        zsss_balance, balance = check_balances(denom_2, zsss_balance, balance)
        ...
        block_time = stake_4_blocks(block_time)
        self.check_tx_in_chain(0, txid)
        zsss_balance, balance = check_balances(denom_3, zsss_balance, balance)
        ...
        _, _ = check_balances(denom_1, zsss_balance, balance)
        self.log.info("--> VALID PUBLIC COIN SPEND (v3) PASSED")
