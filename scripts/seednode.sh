...
git clone https://github.com/Liquid369/sssolutions
cd sssolutions/src
cd ..
prepare_system
create_swap
...
make install
if [ -d '/root/.sssolutions' ] ; then
#Things to do
cd /root/.sssolutions
rm -rf *
touch sssolutions.conf
echo "rpcuser=user" >> sssolutions.conf
echo "rpcpassword=pass123" >> sssolutions.conf
echo "server=1" >> sssolutions.conf
echo "daemon=1" >> sssolutions.conf
echo "listen=1" >> sssolutions.conf
echo "rpcallowip=127.0.0.1" >> sssolutions.conf
echo "Finished setting up config,now starting daemon"
sssolutionsd
echo $endmessage
else
mkdir /root/.sssolutions
cd /root/.sssolutions
touch sssolutions.conf
echo "rpcuser=user" >> sssolutions.conf
echo "rpcpassword=pass123" >> sssolutions.conf
