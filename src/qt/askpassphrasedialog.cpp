...
void AskPassphraseDialog::accept()
{
    ...
    case Mode::Encrypt: {
        ...
        bool ret = openStandardDialog(
                ...
                " <b>" + tr("LOSE ALL OF YOUR SSS") + "</b>!<br><br>" + tr("Are you sure you wish to encrypt your wallet?"),
                tr("ENCRYPT"), tr("CANCEL")
        );
        if (ret) {
            if (newpass1 == newpass2) {
                newpassCache = newpass1;
                SSSGUI* window = static_cast<SSSGUI*>(parentWidget());
...
{
    SSSGUI* gui = static_cast<SSSGUI*>(parentWidget());
...
{
    hide();
    static_cast<SSSGUI*>(parentWidget())->showHide(true);
    openStandardDialog(
            tr("Wallet encrypted"),
            "<qt>" +
            tr("SSS will close now to finish the encryption process. "
