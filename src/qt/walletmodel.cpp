...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl, bool fIncludeDelegations)
{
...

    {
        ...
        if (recipients[0].useSwiftTX && total > sporkManager.GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            Q_EMIT message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 SSS.").arg(sporkManager.GetSporkValue(SPORK_5_MAX_VALUE)),
...
bool WalletModel::createzSSSSpend(
        CWalletTx &wtxNew,
...
bool WalletModel::convertBackzSSS(
        CAmount value,
        std::vector<CZerocoinMint> &vMintsSelected,
        CZerocoinSpendReceipt &receipt)
{
...
static void NotifyzSSSReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzSSSReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzSSSReset.connect(boost::bind(NotifyzSSSReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzSSSReset.disconnect(boost::bind(NotifyzSSSReset, this));
...
bool WalletModel::updateAddressBookPurpose(const QString &addressStr, const std::string& purpose)
{
    ...
    if (address.IsStakingAddress())
        return error("Invalid SSS address, cold staking address");
    CKeyID keyID;
...
{
    if (!address.IsValid())
        return error("Invalid SSS address");

    if (!address.GetKeyID(keyID))
        return error("Unable to get KeyID from SSS address");
...
{
    setMints.clear();
    setMints = pwalletMain->zsssTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus, fWrongSeed);
}
