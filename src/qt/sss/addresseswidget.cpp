...
#include "qt/sss/addresseswidget.h"
#include "qt/sss/forms/ui_addresseswidget.h"
#include "qt/sss/addresslabelrow.h"
#include "qt/sss/addnewaddressdialog.h"
...
AddressesWidget::AddressesWidget(SSSGUI* parent) :
    PWidget(parent),
    ui(new Ui::AddressesWidget)
{
    ...
    // Address
    ui->labelAddress->setText(tr("Enter SSS address"));
...
void AddressesWidget::onStoreContactClicked(){
    if (walletModel) {
        ...
        CBitcoinAddress SSSAdd = CBitcoinAddress(address.toUtf8().constData());
        ...
        if (walletModel->updateAddressBookLabels(SSSAdd.Get(), label.toUtf8().constData(),
                SSSAdd.IsStakingAddress() ? AddressBook::AddressBookPurpose::COLD_STAKING_SEND : AddressBook::AddressBookPurpose::SEND)
                ) {
