...
#include "qt/sss/forms/ui_snackbar.h"
#include "qt/sss/qtutils.h"
#include <QTimer>


SnackBar::SnackBar(SSSGUI* _window, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SnackBar),
    window(_window),
    timeout(MIN_TIMEOUT)
{
    ...
    if (window)
        connect(window, &SSSGUI::windowResizeEvent, this, &SnackBar::windowResizeEvent);
    else {
