...
#include "qt/sss/send.h"
#include "qt/sss/forms/ui_send.h"
#include "qt/sss/addnewcontactdialog.h"
#include "qt/sss/qtutils.h"
#include "qt/sss/sendchangeaddressdialog.h"
#include "qt/sss/optionbutton.h"
#include "qt/sss/sendconfirmdialog.h"
#include "qt/sss/myaddressrow.h"
#include "qt/sss/guitransactionsutils.h"
...
SendWidget::SendWidget(SSSGUI* parent) :
    PWidget(parent),
    ui(new Ui::send),
    ...
    btnContacts(new QPushButton())
{
    ...
    /* Address */
    ui->labelSubtitleAddress->setText(tr("SSS address or contact label"));
    ...
    ui->labelAmountSend->setText("0.00 SSS");
    ...
    setCssProperty(coinIcon, "coin-icon-SSS");
    ...
    // Connect
    connect(ui->pushLeft, &QPushButton::clicked, [this](){onSSSSelected(true);});
    connect(ui->pushRight,  &QPushButton::clicked, [this](){onSSSSelected(false);});
...
void SendWidget::refreshView()
{
    ...
    ui->pushButtonSave->setText(isChecked ? tr("Send SSS") : tr("Send zSSS"));
...
void SendWidget::refreshAmounts()
{
    ...
    bool iszSSS = ui->pushRight->isChecked();
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();

    ui->labelAmountSend->setText(GUIUtil::formatBalance(total, nDisplayUnit, iszSSS));
...
void SendWidget::showHideCheckBoxDelegations()
{
    ...
    // coincontrol is not selected, and we are trying to spend SSS (not zSSS)
    const bool iszSSS = ui->pushRight->isChecked();
    const bool isCControl = CoinControlDialog::coinControl->HasSelected();
    const bool hasDel = cachedDelegatedBalance > 0;

    const bool showCheckBox = !iszSSS && !isCControl && hasDel;
    ...
    if (showCheckBox)
        ui->checkBoxDelegations->setToolTip(
                tr("Possibly spend coins delegated for cold-staking (currently available: %1").arg(
                        GUIUtil::formatBalance(cachedDelegatedBalance, nDisplayUnit, iszSSS))
        );
}
...
void SendWidget::onSendClicked()
{
    ...
    bool sendSSS = ui->pushLeft->isChecked();
    ...
    if ((sendSSS) ? send(recipients) : sendzSSS(recipients)) {
...
bool SendWidget::sendzSSS(QList<SendCoinsRecipient> recipients)
{
    if (!walletModel || !walletModel->getOptionsModel())
        return false;

    if (sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        Q_EMIT message(tr("Spend Zerocoin"), tr("zSSS is currently undergoing maintenance."), CClientUIInterface::MSG_ERROR);
        return false;
    }
    ...
    if (walletModel->sendzSSS(
        ...
        inform(tr("zSSS transaction sent!"));
        zSSSControlDialog::setSelectedMints.clear();
        clearAll(false);
        return true;
    } else {
        QString body;
        if (receipt.GetStatus() == ZSSS_SPEND_V1_SEC_LEVEL) {
            body = tr("Version 1 zSSS require a security level of 100 to successfully spend.");
        } else {
            int nNeededSpends = receipt.GetNeededSpends(); // Number of spends we would need for this transaction
            const int nMaxSpends = Params().GetConsensus().ZC_MaxSpendsPerTx; // Maximum possible spends for one zSSS transaction
        ...
        Q_EMIT message("zSSS transaction failed", body, CClientUIInterface::MSG_ERROR);
        return false;
    }
}
...
void SendWidget::onCoinControlClicked()
{
    if (isSSS) {
        if (walletModel->getBalance() > 0) {
            ...
            inform(tr("You don't have any SSS to select."));
        }
    } else {
        if (walletModel->getZerocoinBalance() > 0) {
            ...
            zSSSControl->exec();
            ui->btnCoinControl->setActive(!zSSSControlDialog::setSelectedMints.empty());
            zSSSControl->deleteLater();
        } else {
            inform(tr("You don't have any zSSS in your balance to select."));
        }
    }
}
...
void SendWidget::onSSSSelected(bool _isSSS)
{
    isSSS = _isSSS;
    setCssProperty(coinIcon, _isSSS ? "coin-icon-SSS" : "coin-icon-zsss");
    refreshView();
...
void SendWidget::onContactMultiClicked()
{
    if (focusedEntry) {
        ...
        CBitcoinAddress SSSAdd = CBitcoinAddress(address.toStdString());
        if (walletModel->isMine(SSSAdd)) {
        ...
        if (dialog->res) {
            ...
            if (walletModel->updateAddressBookLabels(SSSAdd.Get(), dialog->getLabel().toStdString(),
                    AddressBook::AddressBookPurpose::SEND)) {
