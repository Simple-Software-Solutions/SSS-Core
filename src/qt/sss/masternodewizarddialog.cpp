...
#include "qt/sss/masternodewizarddialog.h"
#include "qt/sss/forms/ui_masternodewizarddialog.h"
    ...
    walletModel(model)
{
    ...
    ui->lineEditPort->setPlaceholderText("e.g 6740");
    ...
    if (walletModel->isRegTestNetwork()) {
        ...
        ui->lineEditPort->setEnabled(false);
        ui->lineEditPort->setText("51477");
    } else {
        ui->lineEditPort->setText("6740");
    }
...
bool MasterNodeWizardDialog::createMN()
{
    if (walletModel) {
        ...
        SendCoinsRecipient sendCoinsRecipient(QString::fromStdString(address.ToString()), QString::fromStdString(alias), CAmount(2500) * COIN, "");
            ...
            if (boost::filesystem::exists(pathBootstrap)) {
                ...
                if (lineCopy.size() == 0) {
                    lineCopy = "# Masternode config file\n"
                               ...
                               "# Example: mn1 127.0.0.2:6740 93HaYBVUCYjEMeeH1Y4sBGLALQZE1Yc1K64xiqgX37tGBDQL8Xg 2bcd3c84c84f87eaa86e4e56834c92927a07f9e18718810b92e0d0324456a67c 0"
                               "#";
                }
                lineCopy += "\n";
