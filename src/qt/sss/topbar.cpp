...
#include "qt/sss/topbar.h"
#include "qt/sss/forms/ui_topbar.h"
#include "qt/sss/lockunlock.h"
#include "qt/sss/qtutils.h"
#include "qt/sss/receivedialog.h"
#include "qt/sss/loadingdialog.h"
...
TopBar::TopBar(SSSGUI* _mainWindow, QWidget *parent) :
    ...
    ui(new Ui::TopBar)
{
    ...
    std::initializer_list<QWidget*> lblTitles = {ui->labelTitle1, ui->labelTitleAvailablezSSS, ui->labelTitle3, ui->labelTitle4, ui->labelTitlePendingzSSS, ui->labelTitleImmaturezSSS};
    ...
    setCssProperty({ui->labelAmountTopSSS, ui->labelAmountTopzSSS}, "amount-small-topbar");
    setCssProperty({ui->labelAmountSSS, ui->labelAvailablezSSS}, "amount-topbar");
    setCssProperty({ui->labelPendingSSS, ui->labelPendingzSSS, ui->labelImmatureSSS, ui->labelImmaturezSSS}, "amount-small-topbar");
                            ...
                            const CAmount& delegatedBalance, const CAmount& coldStakedBalance)
{
    ...
    // SSS Total
    CAmount SSSAvailableBalance = balance;
    // zSSS Balance
    CAmount matureZerocoinBalance = zerocoinBalance - unconfirmedZerocoinBalance - immatureZerocoinBalance;

    // Set
    QString totalSSS = GUIUtil::formatBalance(SSSAvailableBalance, nDisplayUnit);
    QString totalzSSS = GUIUtil::formatBalance(matureZerocoinBalance, nDisplayUnit, true);

    // SSS
    // Top
    ui->labelAmountTopSSS->setText(totalSSS);
    // Expanded
    ui->labelAmountSSS->setText(totalSSS);
    ui->labelPendingSSS->setText(GUIUtil::formatBalance(unconfirmedBalance, nDisplayUnit));
    ui->labelImmatureSSS->setText(GUIUtil::formatBalance(immatureBalance, nDisplayUnit));

    // Update display state and/or values for zSSS balances as necessary
    bool fHaveZerocoins = zerocoinBalance > 0;

    // Set visibility of zSSS label titles/values
    ui->typeSpacerTop->setVisible(fHaveZerocoins);
    ui->typeSpacerExpanded->setVisible(fHaveZerocoins);
    ui->labelAmountTopzSSS->setVisible(fHaveZerocoins);
    ui->zerocoinBalances->setVisible(fHaveZerocoins);

    // Top
    ui->labelAmountTopzSSS->setText(totalzSSS);
    // Expanded
    ui->labelAvailablezSSS->setText(totalzSSS);
    ui->labelPendingzSSS->setText(GUIUtil::formatBalance(unconfirmedZerocoinBalance, nDisplayUnit, true));
    ui->labelImmaturezSSS->setText(GUIUtil::formatBalance(immatureZerocoinBalance, nDisplayUnit, true));
}
