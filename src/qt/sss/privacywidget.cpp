...
#include "qt/sss/forms/ui_privacywidget.h"
#include "qt/sss/qtutils.h"
#include "guiutil.h"
#include "qt/sss/txviewholder.h"
...
PrivacyWidget::PrivacyWidget(SSSGUI* parent) :
    PWidget(parent),
    ui(new Ui::PrivacyWidget)
{
    ...
    /* Subtitle */
    ui->labelSubtitle1->setText(tr("Minting zSSS anonymizes your SSS by removing any\ntransaction history, making transactions untraceable "));
    setCssSubtitleScreen(ui->labelSubtitle1);

    ui->labelSubtitle2->setText(tr("Mint new zSSS or convert back to SSS"));
    ...
    ui->lineEditAmount->setPlaceholderText("0.00 SSS ");
    ...
    ui->labelValueDenom1->setText("0x1 = 0 zSSS");
    ...
    ui->labelValueDenom5->setText("0x5 = 0 zSSS");
    ...
    ui->labelValueDenom10->setText("0x10 = 0 zSSS");
    ...
    ui->labelValueDenom50->setText("0x50 = 0 zSSS");
    ...
    ui->labelValueDenom100->setText("0x100 = 0 zSSS");
    ...
    ui->labelValueDenom500->setText("0x500 = 0 zSSS");
    ...
    ui->labelValueDenom1000->setText("0x1000 = 0 zSSS");
    ...
    ui->labelValueDenom5000->setText("0x5000 = 0 zSSS");
    setCssProperty(ui->labelValueDenom5000, "text-body2");

    ui->layoutDenom->setVisible(false);

    // List
    ui->labelListHistory->setText(tr("Last zSSS Movements"));
    ...
    // Only Convert to SSS enabled.
    ui->containerViewPrivacyChecks->setVisible(false);
    onMintSelected(false);

    ui->btnTotalzSSS->setTitleClassAndText("btn-title-grey", "Total 0 zSSS");
    ui->btnTotalzSSS->setSubTitleClassAndText("text-subtitle", "Show denominations of zSSS owned.");
    ui->btnTotalzSSS->setRightIconClass("ic-arrow");

    ui->btnCoinControl->setTitleClassAndText("btn-title-grey", "Coin Control");
    ui->btnCoinControl->setSubTitleClassAndText("text-subtitle", "Select SSS outputs to mint into zSSS.");
    ...
    ui->btnResetZerocoin->setTitleClassAndText("btn-title-grey", "Reset Spent zSSS");
    ui->btnResetZerocoin->setSubTitleClassAndText("text-subtitle", "Reset zerocoin database.");

    connect(ui->btnTotalzSSS, &OptionButton::clicked, this, &PrivacyWidget::onTotalzSSSClicked);
...
void PrivacyWidget::onMintSelected(bool isMint)
{
    QString btnText;
    if (isMint) {
        btnText = tr("Mint zSSS");
        ui->btnCoinControl->setVisible(true);
        ui->labelSubtitleAmount->setText(tr("Enter amount of SSS to mint into zSSS"));
    } else {
        btnText = tr("Convert back to SSS");
        ui->btnCoinControl->setVisible(false);
        ui->labelSubtitleAmount->setText(tr("Enter amount of zSSS to convert back into SSS"));
    }
...
void PrivacyWidget::onTotalzSSSClicked()
{
    bool isVisible = ui->layoutDenom->isVisible();
    if (!isVisible) {
        ui->layoutDenom->setVisible(true);
        ui->btnTotalzSSS->setRightIconClass("btn-dropdown", true);
    } else {
        ui->layoutDenom->setVisible(false);
        ui->btnTotalzSSS->setRightIconClass("ic-arrow", true);
    }
}
...
void PrivacyWidget::onSendClicked()
{
    ...
    if (sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        warn(tr("Zerocoin"), tr("zSSS is currently undergoing maintenance"));
        return;
    }
    ...
    if (!ctx.isValid()) {
        inform(tr("You need to unlock the wallet to be able to %1 zSSS").arg(isConvert ? tr("convert") : tr("mint")));
        return;
    }
...
void PrivacyWidget::mint(CAmount value)
{
    ...
    if (!walletModel->mintCoins(value, CoinControlDialog::coinControl, strError)) {
        ...
        // Mint succeed
        inform(tr("zSSS minted successfully"));
        // clear
...
void PrivacyWidget::spend(CAmount value)
{
    ...
    if (!walletModel->convertBackzSSS(
            value,
            selectedMints,
            receipt
    )) {
        ...
        // Spend succeed
        inform(tr("zSSS converted back to SSS"));
        // clear
        ui->lineEditAmount->clear();
    }
}
...
void PrivacyWidget::updateDenomsSupply()
{
    ...
    ui->btnTotalzSSS->setTitleText(tr("Total %1").arg(GUIUtil::formatBalance(matureZerocoinBalance, nDisplayUnit, true)));
}
