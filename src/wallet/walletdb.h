...
public:
    ...
    bool WriteZSSSSeed(const uint256& hashSeed, const std::vector<unsigned char>& seed);
    bool ReadZSSSSeed(const uint256& hashSeed, std::vector<unsigned char>& seed);
    bool ReadZSSSSeed_deprecated(uint256& seed);
    bool EraseZSSSSeed();
    bool EraseZSSSSeed_deprecated();

    bool WriteZSSSCount(const uint32_t& nCount);
    bool ReadZSSSCount(uint32_t& nCount);
