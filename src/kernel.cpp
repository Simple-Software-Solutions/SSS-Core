...
bool CheckProofOfStake(const CBlock block, uint256& hashProofOfStake, std::unique_ptr<CStakeInput>& stake)
{
    ...
    if (tx.IsZerocoinSpend()) {
        ...
        stake = std::unique_ptr<CStakeInput>(new CzsssStake(spend));
    } else {
        ...
        CSSSStake* SSSInput = new CSSSStake();
        SSSInput->SetInput(txPrev, txin.prevout.n);
        stake = std::unique_ptr<CStakeInput>(SSSInput);
    }
