...
bool CheckZerocoinSpend(const CTransaction& tx, bool fVerifySignature, CValidationState& state, bool fFakeSerialAttack)
{
    ...
    for (const CTxIn& txin : tx.vin) {
        ...
        if (isPublicSpend) {
            ...
            if (!ZSSSModule::parseCoinSpend(txin, tx, prevOut, publicSpend)){
        ...
        if (isPublicSpend) {
            ...
            if (!ZSSSModule::validateInput(txin, prevOut, tx, ret)){
...
bool RecalculateSSSSupply(int nHeightStart, bool fSkipzSSS)
{
    ...
    while (true) {
        ...
        // Rewrite zsss supply too
        if (!fSkipzSSS && pindex->nHeight >= consensus.height_start_ZC) {
            UpdateZSSSSupplyConnect(block, pindex, true);
        }
