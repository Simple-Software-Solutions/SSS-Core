...
bool CSSSStake::InitFromTxIn(const CTxIn& txin)
{
    if (txin.IsZerocoinSpend())
        return error("%s: unable to initialize CSSSStake from zerocoin spend");
...
bool CSSSStake::SetPrevout(CTransaction txPrev, unsigned int n)
{
    this->txFrom = txPrev;
    this->nPosition = n;
    return true;
...
bool CSSSStake::GetTxFrom(CTransaction& tx) const
{
    if (txFrom.IsNull())
        return false;
    tx = txFrom;
...
bool CSSSStake::GetTxOutFrom(CTxOut& out) const
{
...
CAmount CSSSStake::GetValue() const
{
    return txFrom.vout[nPosition].nValue;
}

bool CSSSStake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal)
{
...
CDataStream CSSSStake::GetUniqueness() const
{
...
bool CSSSStake::ContextCheck(int nHeight, uint32_t nTime)
{
