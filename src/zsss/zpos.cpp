// Copyright (c) 2017-2019 The sssolutionsdevelopers
...
bool CLegacyzSSSStake::InitFromTxIn(const CTxIn& txin)
{
    // Construct the stakeinput object
    if (!txin.IsZerocoinSpend())
        return error("%s: unable to initialize CLegacyzSSSStake from non zc-spend");
    ...
    *this = CLegacyzSSSStake(spend);

    // Find the pindex with the accumulator checksum
    if (!GetIndexFrom())
        return error("%s : Failed to find the block index for zsss stake origin", __func__);

    // All good
    return true;
}

CLegacyzSSSStake::CLegacyzSSSStake(const libzerocoin::CoinSpend& spend)
{
...
CBlockIndex* CLegacyzSSSStake::GetIndexFrom()
{
...
CAmount CLegacyzSSSStake::GetValue() const
{
    return denom * COIN;
}

CDataStream CLegacyzSSSStake::GetUniqueness() const
{
...
bool CLegacyzSSSStake::ContextCheck(int nHeight, uint32_t nTime)
{
    const Consensus::Params& consensus = Params().GetConsensus();
    if (nHeight < consensus.height_start_ZC_SerialsV2 || nHeight >= consensus.height_last_ZC_AccumCheckpoint)
        return error("%s : zSSS stake block: height %d outside range", __func__, nHeight);
