...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 6740, 51477));
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all SSS specific functionality (Masternodes, Zerocoin, SwiftX, Budgeting) (0-1, default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-masternodeaddr=<n>", strprintf(_("Set external address:port to get to this masternode (example: %s)"), "128.127.106.235:6740"));
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2019-%i The SSS Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
bool AppInit2()
{
    ...
    // Sanity check
    if (!InitSanityCheck())
        return UIError(_("Initialization sanity check failed. SSS Core is shutting down."));
...
#endif
    // Make sure only a single SSS process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return UIError(strprintf(_("Cannot obtain a lock on data directory %s. SSS Core is probably already running."), strDataDir));
    ...
    while (!fLoaded && !ShutdownRequested()) {
        ...
        do {
            ...
            try {
                ...
                //SSS specific: zerocoin and spork DB's
                ...
                // SSS: load previous sessions sporks if we have them.
                ...
                if (consensus.hashGenesisBlock != uint256("0x000006bbb950f1ff74e9db655586a49a18ceb9107f6b94756b7f48bbd07efcb0")) // && mapBlockIndex.count(consensus.hashGenesisBlock) == 0
                ...
                if (fReindexMoneySupply) {
                    LOCK(cs_main);
                    // Skip zsss if already reindexed
                    RecalculateSSSSupply(1, fReindexZerocoin);
                }
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of SSS Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart SSS Core to complete") << "\n";
        ...
        if (GetBoolArg("-upgradewallet", fFirstRun && !fLegacyWallet)) {
            if (prev_version <= FEATURE_PRE_SSS && pwalletMain->IsLocked()) {
        ...
        if (fFirstRun) {
            if (!fLegacyWallet) {
                ...
                pwalletMain->SetMaxVersion(FEATURE_PRE_SSS);
            }
        ...
        zwalletMain = new CzSSSWallet(pwalletMain);
