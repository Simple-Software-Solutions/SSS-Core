...
- [Development guidelines](#development-guidelines)
    - [General SSS Core](#general-sss-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
SSS Core uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
SSS Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/Simple-Software-Solutions/SSS-Core/blob/master/contrib/valgrind.supp))
...
$ valgrind -v --leak-check=full src/sssd -printtoconsole
```
...
be sent upstream but bugfixes may also be prudent to PR against SSS Core so that they can be integrated
...
- When looking at other's pull requests, it may make sense to add the following section to your `.git/config`
  file:
        ...
        [remote "upstream-pull"]
                ...
                url = git@github.com:sss-core/SSS.git
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `sss-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `sss-cli` and the GUI debug console use this table to determine how to
